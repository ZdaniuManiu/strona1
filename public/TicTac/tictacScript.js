...
startButton.addEventListener('click', function () {
    ...
    if (lastClassWin != "") {
        console.log("Przed delete: " + lastClassWin)
        linia.classList.remove(...lastClassWin.trim().split(" "));

        document.querySelectorAll("td").forEach(element => { element.classList.remove(...lastClassWin.trim().split(" "))})
        
        lastClassWin = ''
        console.log("Po delete: " + lastClassWin)
...
//FUNKCJA NIE PRZYPISANA DO ŻADNEGO PRZYCISKU ODPOWIADA ZA ODBLOKOWANIE PÓL DO GRY PO KLIKNIECIU PRZYCISKU
// MA ODWOLANIE W "ROZPOCZNIJ GRE"
function startGame() {
    ...
    document.querySelectorAll("td").forEach(element => { 
        ...
        if(lastClassWin.includes('active-cell')){
            return
        }else{
        lastClassWin += 'active-cell'}
    })
}
        ...
        for (let i = 0; i < kombinacja.length; i++) {
            if (element.id == kombinacja[i]) {
                if (element.dataset.gracz == "krzyzyk") {
                    element.className += ' wygrane_pola_krzyz'
                    if(lastClassWin.includes('wygrane_pola_krzyz')){
                        break
                    }else{
                        lastClassWin += ' wygrane_pola_krzyz'
                    }
                } else if (element.dataset.gracz == "kolko") {
                    element.className += ' wygrane_pola_kolo'
                    if(lastClassWin.includes('wygrane_pola_kolo')){
                        break
                    }else{
